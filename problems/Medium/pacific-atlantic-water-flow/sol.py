...
class Solution:
	...
	def pacificAtlantic(self, matrix: List[List[int]]) -> List[List[int]]:
		def nbrs(i, j):
			return [(r, c) for r, c in [(i-1, j), (i, j-1), (i, j+1), (i+1, j)] if 0 <= r < R and 0 <= c < C]

		# @cache
		def reachesPacific(i, j, visited):
			if (i, j) in pacific:
				return pacific[(i, j)]
			...
			for r, c in nbrs(i, j):
				if matrix[i][j] >= matrix[r][c] and (r, c) not in visited and reachesPacific(r, c, [(i, j)] + visited):
					pacific[(i, j)] = True
					return pacific[(i, j)]

			pacific[(i, j)] = False
			return pacific[(i, j)]
			# return any([matrix[i][j] >= matrix[r][c] and reachesPacific(r, c) for r, c in nbrs(i, j)])

		# @cache
		def reachesAtlantic(i, j, visited):
			if (i, j) in atlantic:
				return atlantic[(i, j)]
			...
			for r, c in nbrs(i, j):
				if matrix[i][j] >= matrix[r][c] and (r, c) not in visited and reachesAtlantic(r, c, [(i, j)] + visited):
					atlantic[(i, j)] = True
					return atlantic[(i, j)]
		...
		for i in range(R):
			for j in range(C):
				if reachesPacific(i, j, []) == True and reachesAtlantic(i, j, []) == True:
					result.append([i, j])
		return result
